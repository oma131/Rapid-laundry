...
const OurServices = () => {
  ...
  return (
    <div>
    <div className='flex relative overflow-hidden bg-[#ECF0F5] mt-[5rem] justify-center'>
      <div className='flex flex-col px-[1rem] rounded-[0.5rem]  pb-[5rem] my-[3rem] lg:w-[66rem] w-full items-center'>
        <h1 className='text-[#0100BB] text-[2.5rem] px-[2rem]  pb-[6rem]  font-semibold'>Our Services</h1>
        <div className="grid grid-cols-1 lg:grid-cols-6 lg:gap-8">
          {serviceData.map((item, index) => (
            <div className={`p-4 rounded-t-[0.5rem] bg-awhite flex flex-col justify-center items-center w-[20rem] h-[18rem] ${index == 3 ? 'col-span-2 col-start-2' : 'col-span-2 '}`} key={item.id}>
      ...
      <div className='lg:h-[12rem] h-[8rem] z-10 lg:w-[12rem]  w-[8rem] relative left-[12rem] rounded-full opacity-30 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#F4F4F6] via-[#C6C6FA]  to-[#9291FE]'></div>
     <div className='lg:h-[9rem] h-[4.5rem] z-0 lg:w-[9rem] w-[4.5rem] relative top-[0.5rem] left-[6rem]  rounded-full opacity-30 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#F4F4F6] via-[#C6C6FA]  to-[#9291FE]'></div>
     <div className='lg:h-[6rem] h-[3rem] z-20 lg:w-[6rem] w-[3rem] relative top-[9rem] rounded-full opacity-60 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#F4F4F6] via-[#C6C6FA]  to-[#9291FE]'></div>
     <div className='lg:h-[2rem] h-[3rem] z-20 lg:w-[2rem] w-[3rem] relative top-[10rem] right-[15rem] rounded-full opacity-60 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#F4F4F6] via-[#C6C6FA]  to-[#9291FE]'></div>
     <div className='lg:h-[1rem] h-[3rem] z-20 lg:w-[1rem] w-[3rem] relative top-[12rem] right-[18rem] rounded-full opacity-60 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#F4F4F6] via-[#C6C6FA]  to-[#9291FE]'></div>
